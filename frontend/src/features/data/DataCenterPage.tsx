...
type Status = {
  providers: Record<string, { status: string; required: boolean }>;
  snapshot: Meta | null;
};
...
export function DataCenterPage() {
  ...
  return <>
    ...
    <section className="provider-grid">{query.data && Object.entries(query.data.providers).map(([name, provider]) => <article key={name}><span className={`provider-status ${provider.status}`} /><div><strong>{name}</strong><small>{provider.required ? "核心数据源" : "可选增强"}</small></div><b>{provider.status.replaceAll("_", " ")}</b></article>)}</section>
    ...
    <section className="panel data-note"><h3>覆盖与降级规则</h3><p>行情失败时使用最后一次有效快照并标记为 stale；没有快照时显示不可用，不把空值当作 0。</p><p>问财未配置不会影响核心行情、机会漏斗、个股价格和观察清单。配置后可增强财务、公告、研报与行业信息。</p>{snapshot?.errors.length ? <div className="provider-errors"><strong>最近错误</strong>{snapshot.errors.map((error) => <p key={error}>{error}</p>)}</div> : <p className="positive">当前快照没有标准化错误。</p>}</section>
  </>;
}

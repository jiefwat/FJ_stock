...
const dossier = {
  quote: { symbol: "SH.600519", code: "600519", name: "贵州茅台", price: 1253, change_pct: -0.48, amount: 1, turnover_rate: 1, volume_ratio: null, pe: 23, pb: 7, market_cap: 1, net_flow: null, sector: "白酒Ⅱ" },
  ...
  financial_health: {
    ...
    score: 68,
    score_impact: 3,
    conclusion: "盈利和现金质量较稳",
    report_date: "2026-06-30",
    risks: ["利润增速需要继续观察"],
    error: null,
    periods: [{
      report_date: "2026-06-30", report_label: "2026中报", report_type: "中报",
      revenue: 90703000000, revenue_yoy: 1.47, net_profit: 44517000000, net_profit_yoy: -1.95,
      roe: 18.2, gross_margin: 91.3, operating_cash_flow_per_share: 16.8,
      cash_receipts_to_revenue: 98.4, debt_to_assets: 13.1,
    }],
  },
  news_sentiment: {
    available: true,
    window_days: 30,
    conclusion: "近30天有1条明确风险",
    positive_count: 1,
    negative_count: 1,
    neutral_count: 0,
    hard_risk_count: 1,
    score_impact: -3,
    risks: ["公司被证监会立案调查（立案调查）"],
    error: null,
    items: [{
      id: "eastmoney-news:1", title: "公司被证监会立案调查", summary: "公司收到立案告知书。",
      media: "证券时报", url: "https://example.com/stock-news", published_at: "2026-08-15T08:00:00Z",
      sentiment: "negative", hard_risk: true, hard_risk_terms: ["立案调查"],
    }],
  },
  analysis_dimensions: [
...
it("explains when a stock dossier is opened from opportunity leads", async () => {
  ...
  expect(sourceNote.getByText("候选")).toBeInTheDocument();
...
it("shows the evidence ledger and keeps Ask Stock as the primary action", async () => {
  ...
  expect(screen.getByText(/信息完整度 55%/)).toBeInTheDocument();
...
it("puts company, news, price, and invalidation in one decision view", async () => {
  ...
  expect(drivers.getByText("消息风险")).toBeInTheDocument();
  expect(drivers.getByText("近30天有1条明确风险")).toBeInTheDocument();
  expect(drivers.getByText("价格状态")).toBeInTheDocument();
  expect(drivers.getByText("偏强震荡")).toBeInTheDocument();
  expect(drivers.getByText("改变决定")).toBeInTheDocument();
  expect(drivers.getByText(/跌破近 20 日支撑 1168\.00/)).toBeInTheDocument();
  expect(drivers.getByRole("link", { name: "公司被证监会立案调查" })).toHaveAttribute(
    "href",
    "https://example.com/stock-news",
  );
  ...
  await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => String(input).includes("/evidence"))).toBe(true));
});

it("keeps the stock decision when finance and news are unavailable", async () => {
  renderPage(true, "/stocks?symbol=SH.600519", evidence, {
    ...dossier,
    financial_health: { available: false, score: null, score_impact: 0, conclusion: "财报暂不可用", report_date: null, risks: [], periods: [], error: "timeout" },
    news_sentiment: { available: false, window_days: 30, conclusion: "新闻暂不可用", positive_count: 0, negative_count: 0, neutral_count: 0, hard_risk_count: 0, score_impact: 0, risks: [], items: [], error: "timeout" },
  });

  expect(await screen.findByText("贵州茅台")).toBeInTheDocument();
  const drivers = within(screen.getByLabelText("四维决策依据"));
  expect(drivers.getByText("财报暂不可用")).toBeInTheDocument();
  expect(drivers.getByText("新闻暂不可用")).toBeInTheDocument();
...
it("shows cited filings, research reports, and themes without changing the score", async () => {
  ...
  await screen.findByText("查看专业数据");
...
it("formats the generated stock conclusion into a scannable analyst brief", async () => {
  ...
  expect(brief.getByText("最近价格表现")).toBeInTheDocument();
  expect(brief.getByText("可能收益和风险")).toBeInTheDocument();
  expect(brief.getByText("当前价格贵不贵")).toBeInTheDocument();
  expect(brief.getByText("买卖是否方便")).toBeInTheDocument();
  expect(brief.getByText("公司经营情况")).toBeInTheDocument();
  expect(brief.getByText("可能推动股价的消息")).toBeInTheDocument();
  expect(brief.getByText("资金/行业")).toBeInTheDocument();
  expect(brief.getByText("操作纪律")).toBeInTheDocument();
  expect(brief.getByText("处理纪律")).toBeInTheDocument();
  expect(brief.getByText("主要风险")).toBeInTheDocument();
  expect(brief.getByText("后续跟踪")).toBeInTheDocument();
  expect(brief.getByText(/持续跟踪：把 1168.63 作为跟踪放弃线/)).toBeInTheDocument();
...
it("surfaces an analyst action map before the deep evidence sections", async () => {
  ...
  expect(actionMap.getByText("收盘价位于 20 日平均价格线之上")).toBeInTheDocument();
  expect(actionMap.getByText("主要担心什么")).toBeInTheDocument();
  expect(actionMap.getByText("近20天平均价 低于 近60天平均价")).toBeInTheDocument();
  expect(actionMap.getByText("还缺什么信息")).toBeInTheDocument();
  expect(actionMap.getByText("没有明显缺口，继续按现有信息观察")).toBeInTheDocument();
  expect(actionMap.getByText("后续跟踪")).toBeInTheDocument();
  expect(actionMap.getByText("等待价格回落条件出现后自动重算结论")).toBeInTheDocument();
...
it("shows direct investment advice with horizontal and vertical comparisons", async () => {
  ...
  expect(advice.getByText("参与")).toBeInTheDocument();
  expect(advice.getByText("退出")).toBeInTheDocument();
  expect(advice.getByText("落袋")).toBeInTheDocument();

...
export function LimitLadderPage() {
  ...
  return <>
    ...
    <AsyncState loading={query.isLoading} error={query.error as Error | null}>{query.data && <>
      ...
      {query.data.available && (levels.length ? <section className="ladder-board" aria-label={`${mode === "up" ? "涨停" : "跌停"}梯队列表`}>{levels.map((level) => <article key={level.streak} className="ladder-column"><header><span>{level.label}</span><strong>{level.stocks.length}</strong></header><div>{level.stocks.map((item) => <Link key={item.quote.symbol} to={`/stocks?symbol=${encodeURIComponent(item.quote.symbol)}&from=limit-ladder`}><div><span><strong>{item.quote.name}</strong><small>{item.quote.symbol} · {item.quote.sector ?? "行业待补"}</small></span><b className={mode === "up" ? "up" : "down"}>{pct(item.quote.change_pct)}</b></div><p>{item.one_word == null ? "一字状态待原始日线" : item.one_word ? "一字板" : "非一字"}<em>{item.limit_pct.toFixed(0)}% 价格限制</em></p><footer><span>置信 {percent(item.confidence * 100)}</span><b>{item.quote.amount == null ? "成交 N/A" : `${fmt(item.quote.amount / 100_000_000)} 亿成交`}</b></footer></Link>)}</div></article>)}</section> : <section className="structure-empty"><strong>当前筛选没有{mode === "up" ? "涨停" : "跌停"}股票</strong><p>这是真实空结果；可切换方向或清除行业、板块筛选。</p></section>)}
      <section className="ladder-industry"><header><strong>行业分布</strong><small>用于发现梯队扩散，不等于参与建议</small></header><div>{Object.entries(query.data.industry_distribution).map(([name, count]) => <button key={name} className={industry === name ? "active" : ""} onClick={() => setIndustry(industry === name ? "all" : name)}><span>{name}</span><strong>{count}</strong></button>)}</div></section>

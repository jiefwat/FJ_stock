...
export function LimitLadderPage() {
  ...
  return <>
    <WorkbenchPageHeader eyebrow="LIMIT STRUCTURE" title="连板梯队" description="先选涨跌方向与市场范围，再看梯队高度、行业扩散和逐只证据。" status={query.data ? <DataStamp meta={query.data.meta} /> : undefined} />
    <PageTaskRail label="连板梯队" steps={[
      { id: "ladder-filters", label: "限定范围", detail: "方向、行业与板块" },
      { id: "ladder-summary", label: "判断强度", detail: "数量、高度与置信度" },
    ...
    <AsyncState loading={query.isLoading} error={query.error as Error | null}>{query.data && <>
      ...
      <section id="ladder-summary" className={`ladder-summary ${mode}`} aria-label="梯队摘要"><article><span>{mode === "up" ? "涨停股票" : "跌停股票"}</span><strong>{query.data.total}</strong></article><article><span>最高梯队</span><strong>{query.data.max_streak ? `${query.data.max_streak} 板` : "无"}</strong></article><article><span>分析置信度</span><strong>{percent(query.data.confidence * 100)}</strong><small>证据覆盖，不是上涨概率</small></article><article><span>封单数据</span><strong>不可用</strong><small>无盘口证据，不作估算</small></article></section>

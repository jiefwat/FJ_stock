...
- 新闻/公告/财务/舆情链路：`scripts/enrich_tdx_snapshot.py` 把 AKShare 东方财富个股新闻、AKShare/外部 JSON 市场新闻、Tushare `daily_basic`、Tushare `fina_indicator`、可选 Tushare `moneyflow` 写入快照；`scripts/run_daily_pipeline.py` 把巨潮/CNInfo 公告写入 `reports/announcements/` 并同步回填 `stocks[code].announcements`。Web 个股页必须消费快照里的 `news_items`、`announcements`、`fundamental_metrics`、`valuation`、`fund_flow_detail`；每日大盘必须展示市场舆情正负面和最新风险标题。缺任一块时只降级提示，不伪造结论。
- `longbridge.mcp`：Codex 会话可用 Longbridge MCP 查询市场新闻、个股新闻、异动事件、市场温度和财经日历；`scripts/import_mcp_market_intelligence.py` 负责把 MCP JSON 输出转换为快照 `market_news`，并写入 `mcp_market_news_refresh` 供 Web 数据中台展示渠道、更新时间、导入数量和缺口预警。项目运行时不直接调用聊天 MCP，避免页面阻塞和不可复现。
...
- Web 层必须把数据新鲜度作为硬闸门：非 sample 数据的个股/大盘交易日落后于预期最近交易日，或 `reports/daily/pipeline.status` 的 `generated_at` 超过 8 小时时，候选排序、评分展示和“按今天盘面执行”自动暂停；页面只允许进入补数据和条件化复核。
- Web 数据质量页必须读取 `reports/daily/pipeline.status`，展示 `07:00/09:00/13:00/15:00` 四个固定刷新点、最近运行、分步骤结果和处理建议，避免旧数据被误认为当天数据。
- 大盘和机会研究快照必须晚于最近一次成功的主流水线；页面打开时若发现研究快照落后，立即使用最新本地行情生成事实结果，不能等待外部研究，也不能展示刷新中的半成品。
- 盘中行情可以使用当天实时交易日，但 Tushare 当日日线在收盘后不会立即稳定发布，因此 18:00 前按上一完整交易日验收；外部研究只补强本地事实，不得替代或覆盖本地大盘与候选数据。
- 主快照、日报和 HTML 采用 run-specific staging，只有数据链验收通过的版本才能进入正式路径；主快照使用 `os.replace` 原子发布，并以 snapshot 路径级进程锁阻止定时任务和手工刷新重叠。`pipeline.status`、研究快照和 Web 通过 `snapshot_version` 绑定，刷新中、失败或并发补跑不得覆盖最后完整版本。
- 热门机会最多展示 Top10，按“可进入投资候选 / 等待确认 / 反弹观察”分层；反弹观察至少 35 分，脉冲、过热、陈旧行情和价格不可靠标的仍排除，不能为凑数降低数据闸门。

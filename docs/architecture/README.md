...
所有动态请求先检查快照目标文件的 inode、大小和纳秒级修改时间。指纹未变化时复用同一份解析结果；上游原子替换文件后指纹变化，下一次请求自动重新解析，因此行情更新不需要重启进程。快照缺失或损坏时，页面仍返回 200 并展示明确中断状态；API 返回 503 和 `status=unavailable`，不会用旧数据或演示价格补位。

生产环境的 `/opt/aster-market/data/market_snapshot.json` 必须指向持续刷新的上游文件，不能是部署时的一次性副本。上游采用原子替换文件时，符号链接仍按路径读取最新版本。

## 模块职责

- `models.py`：指数、主题、股票、K 线、估值、资金、事件和市场快照的不可变模型。
- `snapshot.py`：只读 JSON 适配器，合并股票主档案与候选档案；不发起网络请求。
- `snapshot_cache.py`：按目标文件指纹复用解析结果，并在原子替换后自动失效。
- `analysis.py`：大盘四维证据、参与许可、主线判定、机会阶段、股票趋势/动量/波动和搜索。
- `presenter.py`：供应商中立页面合同和决策简报组装。
- `ui.py`、`ui_modules.py`：研判链、主线梯队、分析甲板 HTML 渲染和外部字符串转义。

...
主题读取 `name`、`pct_chg`、`advancing_ratio`、`amount_change`、`consecutive_days` 和 `high_divergence`。参与度、连续性或分歧字段缺失时保留为 `null`，不得把缺失证据解释成 0 或无分歧。候选读取代码、名称、主题和最后一根 K 线；若候选自身没有 K 线，会按代码读取 `stocks`。市场事件只读取时间、来源、标题和摘要。
...
- 参与许可：扩张且风险可控为主动跟踪；轮动且风险未升高为结构确认；收缩或风险升高为防守等待。
- 主线判定：扩张市扩散主题可确认主线；轮动市扩散或加速主题为候选主线；防守市场中的强势主题统一降级为逆势异动。
- 市场机会：按主题强度排序，阶段为扩散、加速、分歧、方向分歧或观察；防守市场统一降级为逆势异动，每条带失效条件。
...
`deploy/link-live-snapshot.sh` 将该稳定路径连接到持续刷新的上游快照。应用每次请求检查目标文件指纹；
指纹不变时复用内存解析结果，上游完成原子替换后下一次请求自动重新解析，无需重启服务。
